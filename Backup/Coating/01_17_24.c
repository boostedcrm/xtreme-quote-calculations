...
if(Terrazzo)
{
	if(Terrazzo_Deal_Id == "")
	{
		...
		deal_map.put("Type_of_Service","Terrazzo");
		// Terrazzo/VCT & LVT/Tile & Grout
		deal_map.put("Quote_Type","Coating Terrazzo");

        
        deal_map.put("Vendor_Type1", ifnull(record_resp.get("Vendor_Type"),""));
        deal_map.put("SourceForm", "Coating"); // Coating, Concrete, Honing
        deal_map.put("Bid_Date", Create_Date); // Create_Date1, Create_Date -datetime
        deal_map.put("SquareFeet", ifnull(record_resp.get("Terrazzo_SQFT"), 0) ); // T_G_SQFT, Terrazzo_SQFT, VCT_LVT_SQFT
        deal_map.put("BidID", record_id + "" );
        deal_map.put("Quote_Status", "New");
        deal_map.put("Sales_Person", owner_id);
        deal_map.put("Assigned_To", owner_id);
        deal_map.put("Sent_for_Review", false);
...
if(Tile_Grout)
{
	if(Tile_Grout_Deal_Id == "")
	{
		...
		deal_map.put("Est_Perform_Date1",Est_Perform_Date);
		deal_map.put("Estimated_Perform_Date",Est_Perform_Date);
		deal_map.put("Type_of_Service","Tile & Grout");
		// Terrazzo/VCT & LVT/Tile & Grout
		deal_map.put("Quote_Type","Coating Tile & Grout");
		// Coating Terrazzo / Coating VCT & LVT / Coating Tile & Grout
...
if(VCT_LVT)
{
	if(VCT_LVT_Deal_Id == "")
	{
		...
		deal_map.put("Est_Perform_Date1",Est_Perform_Date);
		deal_map.put("Estimated_Perform_Date",Est_Perform_Date);
		deal_map.put("Type_of_Service","VCT & LVT");
		// Terrazzo/VCT & LVT/Tile & Grout
		deal_map.put("Quote_Type","Coating VCT & LVT");
		// Coating Terrazzo / Coating VCT & LVT / Coating Tile & Grout
